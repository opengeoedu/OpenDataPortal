  ...
  if( (!inherits(datagroup, "SharedData") && dim(datagroup)[1]>0) || (inherits(datagroup, "SharedData") && dim(datagroup$data())[1]>0)){
    m <-addMarkers(
      ...
      icon =  ~ icons(
        ...
        iconAnchorX = "0px",#iconWidth/4,
        iconAnchorY = "0px" #iconHeight/4
      ),
      ...
      clusterId = clusterId,
      labelOptions = labelOptions(textOnly = FALSE,noHide = FALSE,#offset = c(3,-8),
                                  className = "mymarker2"),#, className = "needAbsolute",offset= c(-8, -8)),
      data =  datagroup
    )
  }
  return(m)
...
createLegend <- function(colorlf, categories) {
...
createBaseMap <- function(portale_shared = NULL, htmlLegend=NULL, measure=TRUE, providertiles=providers$CartoDB.PositronNoLabels) {
  m <-
   ...
   addResetMapButton() %>%
    ...
    if(measure)
      m <- addMeasure(m, position = "bottomright", primaryLengthUnit = "meters", localization = "de")
    if(!is.null(htmlLegend))
      m <-  addControl(m, htmlLegend ,position = "topright", className="html-legend")
  return(m)
}

addAdminBoundaries <- function(m, g6bounds, polygon_fill_color, s6bounds, a6bounds, g4bounds, polygon_fill_opacity, s4bounds, a4bounds, labeladm4opts, g2bounds, s2bounds, a2bounds) {
  m <- m %>%
    addPolygons(data = g6bounds, color = "grey", fillColor = polygon_fill_color, weight = 1, group = "adm6",fill=TRUE,label = g6bounds$localname, fillOpacity = 0, options = pathOptions(interactive = TRUE, className = "polygonShape")) %>%
    ...
    addPolygons(data = a6bounds, color = "grey", fillColor = polygon_fill_color, weight = 1,label = a6bounds$localname, group = "adm6",fill=TRUE, fillOpacity = 0, options = pathOptions(interactive = TRUE, className = "polygonShape")) %>%
    ...
    # 
    addPolygons(data = g4bounds, color = "grey", fillColor = polygon_fill_color, weight = 2, label = g4bounds$localname, group = "adm4", fill=TRUE, fillOpacity = polygon_fill_opacity, options = pathOptions(interactive = TRUE, className = "polygonShape")) %>%
    addPolygons(data = s4bounds, color = "grey", fillColor = polygon_fill_color, weight = 2, label = s4bounds$localname, group  = "adm4", fill=TRUE, fillOpacity = polygon_fill_opacity, options = pathOptions(interactive = TRUE, className = "polygonShape")) %>%
    addPolygons(data = a4bounds, color = "grey", fillColor = polygon_fill_color, weight = 2, label = a4bounds$localname, group = "adm4", fill=TRUE, fillOpacity = polygon_fill_opacity, options = pathOptions(interactive = TRUE, className = "polygonShape")) %>%
    addLabelOnlyMarkers(data = gPointOnSurface(geometry(g4bounds), byid = TRUE), label = g4bounds$localname, group = "adm4_labels", labelOptions = labeladm4opts, options = markerOptions(interactive = FALSE)) %>%
    addLabelOnlyMarkers(data = gPointOnSurface(geometry(a4bounds), byid = TRUE), label = a4bounds$localname, group = "adm4_labels", labelOptions = labeladm4opts, options = markerOptions(interactive = FALSE)) %>%
    # 
    ...
    # 
    addPolygons(data = g2bounds,weight = 2, color= "black",group = "adm2", fill = FALSE, options = pathOptions(interactive = FALSE)) %>%  #style = "color:#03F"
    addPolygons(data = s2bounds,weight = 2, color= "black",group  = "adm2", fill = FALSE, options = pathOptions(interactive = FALSE)) %>%
    addPolygons(data = a2bounds,weight = 2, color= "black",group = "adm2", fill = FALSE, options = pathOptions(interactive = FALSE)) 
  return(m)
}

addCities <- function(m, pplc, ppla, ppl, polygon_fill_color) {
  m %>%
    ### Adding markers for poplated places, each with a small circle and a separate label-only-marker
    ##(pplc: capitals, ppla: seat of a first-order administrative division, ppl: other populated places, source: geonames.org)
    addLabelOnlyMarkers(data=pplc, label = pplc$name, labelOptions = labelOptions(noHide = TRUE, opacity = 1, textOnly = TRUE, textsize = "10pt", className = "ppl"), options = markerOptions(interactive = FALSE)) %>%
    addLabelOnlyMarkers(data=ppla, label = ppla$name, labelOptions = labelOptions(noHide = TRUE, opacity = 1, textOnly = TRUE, textsize = "10pt", className = "ppl"), group = "adm4_labels", options = markerOptions(interactive = FALSE)) %>%
    addLabelOnlyMarkers(data=ppl, label = ppl$name, labelOptions = labelOptions(noHide = TRUE, opacity = 1, textOnly = TRUE, textsize = "10pt", className = "ppl"), group = "adm6_labels", options = markerOptions(interactive = FALSE)) %>%
    addCircleMarkers(data=pplc, color = "red",radius=1, options = markerOptions(interactive = FALSE)) %>%
    addCircleMarkers(data=ppla, color = "black", radius=1, group = "adm4_labels", options = markerOptions(interactive = FALSE)) %>%
    addCircleMarkers(data=ppl, color = polygon_fill_color ,radius=1, group = "adm6_labels", options = markerOptions(interactive = FALSE))
  return(m)
}
...
createMap <- function(portale, table_meta, crosstalk_group = "portale", clustering = TRUE, layerControls = TRUE, polygon_fill_color = "#696969", polygon_fill_opacity = 0.2) {
  ...
  labeladm6opts <- labelOptions(textOnly = TRUE, noHide = TRUE, direction = "bottom", opacity = 1, textsize = "10pt", className = "labeladm6")
  labeladm4opts <- labelOptions(textOnly = TRUE, noHide = TRUE, direction = "bottom", opacity = 1, textsize = "13pt", className = "labeladm4")
  ...
  htmlLegend <- createLegend(colorlf, categories)
  m <- createBaseMap(portale_shared, htmlLegend = htmlLegend)
  m <- addAdminBoundaries(m, g6bounds, polygon_fill_color, s6bounds, a6bounds, g4bounds, polygon_fill_opacity, s4bounds, a4bounds, labeladm4opts, g2bounds, s2bounds, a2bounds)

  m <- addCities(m, pplc, ppla, ppl, polygon_fill_color)
   
    # 
    # 
    # 
    #addCircleMarkers(data=pplc, color = "red",radius=1, group = "adm2_labels", options = markerOptions(interactive = FALSE),label = pplc$name, labelOptions = labelOptions(noHide = TRUE, textsize = 13, className = "ppl_label" )) %>%
    #addCircleMarkers(data=ppla, color = "black", radius=1, group = "adm4_labels", options = markerOptions(interactive = FALSE, zIndexOffset = -500), label = ppla$name, labelOptions = labelOptions(noHide = TRUE, zoomAnimation = FALSE)) %>%
    #addCircleMarkers(data=ppl, color = polygon_fill_color ,radius=1, group = "adm6_labels", options = markerOptions(interactive = FALSE, zIndexOffset = -500), label = ppl$name, labelOptions = labelOptions(noHide = TRUE, zoomAnimation = FALSE)) %>%
    #hideGroup("adm6") %>% hideGroup("adm5") %>% hideGroup("adm6_labels") %>% hideGroup("adm4_labels")  %>%
  m <-  m %>% groupOptions("adm6_labels", zoomLevels = 9:18) %>%
